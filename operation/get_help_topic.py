...
class GetData(object):
    def __init__(self,crawl_number, data_path, url_file, error_url_file, date_time,post_table_name, comment_first_table_name,
                   comment_second_table_name,doctor_table_name,post_type,doctor_url_split,afresh_post_url_file = True):
    ...
    def get_data(self):
        ...
        if self.afresh_post_url_file == True:
            get_post = GetPostUrl(file_path=file_path, case_experience_name=self.url_file, help_topic_name=self.url_file)
            get_post.get_help_topic_post_url()
        else:
            pass

        file = open(file_path + self.url_file, 'r')
        reader = csv.reader(file)
        file_error = open(file_path + self.error_url_file, 'wb')
        file_error.close()
        error_number = 0
        ...
        for line in reader:
            ...
            if post_comment != False:
                ...
                print '#### Error!!'
                file_error = open(file_path + self.error_url_file, 'a')
